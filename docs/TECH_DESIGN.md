...
- **Embeddings and reranking, local on Apple Silicon over an API.** `bge-small` embeddings and the `bge-reranker` run on the Mac's GPU (MPS) for free. Because the corpus and the per-query candidate set are small, even CPU inference in the deployed environment is fast enough, so local models don't force a heavier host. This keeps the retrieval ladder cost-free and offline-capable; the only calls that must go through the paid gateway are the LLM's.
